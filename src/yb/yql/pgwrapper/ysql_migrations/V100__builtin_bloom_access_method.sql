-- Register built-in bloom index access method for existing clusters.
...
INSERT INTO pg_catalog.pg_opfamily (
...
SELECT 8506, 8502, 23, 23, 1, 's', o.oid, 8500, 0
...
SELECT 8507, 8503, 25, 25, 1, 's', o.oid, 8500, 0
FROM pg_catalog.pg_operator o
...
SELECT 8508, 8502, 23, 23, 1, p.oid
...
SELECT 8509, 8503, 25, 25, 1, p.oid
FROM pg_catalog.pg_proc p
...
BEGIN
  IF NOT EXISTS (
    SELECT FROM pg_catalog.pg_depend
      WHERE refclassid = 1255 AND refobjid = 8501
  ) THEN
