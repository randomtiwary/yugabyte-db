...
generate_queries_for_path_pattern(RangeTblEntry *rte, List *path_pattern)
{
	 ...
	 */
	foreach(lc_pgq1, path_pattern)
	{
		GraphElementPattern	   *gep = lfirst_node(GraphElementPattern, lc_pgq1);
		...
		foreach(lc_other, path_factors)
		{
			struct path_factor *other = (struct path_factor *) lfirst(lc_other);
	 ...
	 */
	foreach(lc_pf, path_factors)
	{
		struct path_factor *pf = (struct path_factor *) lfirst(lc_pf);
...
generate_queries_for_path_pattern_recurse(RangeTblEntry *rte, List *pathqueries, List *cur_path, List *path_elem_lists, int elempos)
{
	...
	foreach(lc_pe, path_elems)
	{
		struct path_element *pe = (struct path_element *) lfirst(lc_pe);
...
generate_query_for_graph_path(RangeTblEntry *rte, List *graph_path)
{
	...
	foreach(lc, vars)
	{
		...
		RangeTblEntry *rte_var = rt_fetch(var->varno, path_query->rtable);

		/* Must offset the attnum to fit in a bitmapset */
		rte_var->selectedCols = bms_add_member(rte_var->selectedCols,
...
generate_query_for_empty_path_pattern(RangeTblEntry *rte)
{
	 ...
	 */
	foreach(lc_pgq2, rte->graph_table_columns)
	{
		TargetEntry	   *te = lfirst_node(TargetEntry, lc_pgq2);
	...
	*rtable = lappend(*rtable, pni->p_rte);
	...
	if (rarg == NULL)
	{
		...
		if (targetlist)
		{
			...
			*targetlist = NIL;
			foreach(lc_pgq3, lquery->targetList)
			{
				TargetEntry *tle = lfirst_node(TargetEntry, lc_pgq3);
...
get_labels_for_expr(Oid propgraphid, Node *labelexpr)
{
	...
	ListCell   *lc;
		...
		label_oids = NIL;
		foreach(lc, label_exprs)
			{
				GraphLabelRef	   *glr = lfirst_node(GraphLabelRef, lc);
...
get_path_elements_for_path_factor(Oid propgraphid, struct path_factor *pf)
{
	...
	foreach(lc_labeloid, label_oids)
	{
		Oid			labeloid = lfirst_oid(lc_labeloid);
		bool		found = false;
...
replace_property_refs_mutator(Node *node, struct replace_property_refs_context *context)
{
	...
	else if (IsA(node, GraphPropertyRef))
	{
		...
		foreach(lc_m, context->mappings)
		{
			struct path_element *m = (struct path_element *) lfirst(lc_m);
		 ...
		 */
		foreach(lc_labeloid, mapping_factor->labeloids)
		{
			Oid			labeloid = lfirst_oid(lc_labeloid);
			...
			if (OidIsValid(elem_labelid))
			{
				...
				n = stringToNode(TextDatumGetCString(SysCacheGetAttr(PROPGRAPHLABELPROP, tup, Anum_pg_propgraph_label_property_plpexpr, NULL)));
		...
		if (!n)
		{
			...
			foreach(lc_loid, unrelated_labels)
			{
				Oid			loid = lfirst_oid(lc_loid);
...
build_edge_vertex_link_quals(HeapTuple edgetup, int edgerti, int refrti, Oid refid, AttrNumber catalog_key_attnum, AttrNumber catalog_ref_attnum, AttrNumber catalog_eqop_attnum)
{
	...
	datum = SysCacheGetAttr(PROPGRAPHELOID, edgetup, catalog_key_attnum, NULL);
	deconstruct_array(DatumGetArrayTypeP(datum), INT2OID, 2, true, 's', &d1, NULL, &n1);

	datum = SysCacheGetAttr(PROPGRAPHELOID, edgetup, catalog_ref_attnum, NULL);
	deconstruct_array(DatumGetArrayTypeP(datum), INT2OID, 2, true, 's', &d2, NULL, &n2);

	datum = SysCacheGetAttr(PROPGRAPHELOID, edgetup, catalog_eqop_attnum, NULL);
	deconstruct_array(DatumGetArrayTypeP(datum), OIDOID, 4, true, 'i', &d3, NULL, &n3);
...
get_element_property_expr(Oid elemoid, Oid propoid, int rtindex)
{
	...
	while (HeapTupleIsValid(labeltup = systable_getnext(scan)))
	{
		...
		if (!proptup)
			continue;
		n = stringToNode(TextDatumGetCString(SysCacheGetAttr(PROPGRAPHLABELPROP, proptup, Anum_pg_propgraph_label_property_plpexpr, NULL)));
