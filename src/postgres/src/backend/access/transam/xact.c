...
YbCommitTransactionCommandIntermediate(void)
{
	 ...
	 * can restore it on the next transaction. Intermediate commits clear
	 * ddl_transaction_state (including is_top_level_ddl_active) when a plain
	 * transaction containing DDL is committed. Without restoring that flag,
	 * multi-transaction DDLs such as REINDEX can incorrectly allow query-layer
	 * retries under READ COMMITTED after the first intermediate commit.
	 */
